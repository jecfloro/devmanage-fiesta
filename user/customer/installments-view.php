...
try {
    ...
    $files = $conn->prepare("SELECT * FROM msc_uploads WHERE FK_appsysUsers = '$usercode' AND FK_mn_installments = '$iid'");
    $files->execute();
    $cfiles = $files->rowCount();

    $schedule = $conn->prepare("SELECT * FROM mm_schedule WHERE FK_appsysUsers = '$usercode' AND FK_mn_installments = '$iid'");
    $schedule->execute();
    $cschedule = $schedule->rowCount();

    $locationlist = $conn->prepare("SELECT * FROM mm_location WHERE FK_appsysUsers = '$usercode' AND isDefault = 1");
    ...
    $schedulebal = $conn->prepare("SELECT * FROM mm_schedule WHERE FK_mn_installments = '$iid' AND FK_appsysUsers = '$usercode' AND STATUS IS NULL OR STATUS = 'BALANCE' ORDER BY `order` ASC");
    $schedulebal->execute();
    ...
    $payments = $conn->prepare("SELECT * FROM mm_payments WHERE FK_appsysUsers = '$usercode' AND FK_mn_installments = '$iid'");
    $payments->execute();
