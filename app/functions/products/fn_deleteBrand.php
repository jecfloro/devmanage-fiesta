...
$ii_brandiddelete = $_POST['ii_brandiddelete'];

try {
    $conn = new PDO("mysql:host=$fa_dbserver;dbname=$fa_dbname", $fa_dbuser, $fa_dbpassword);
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    $product = $conn->prepare("SELECT * FROM msc_products WHERE productBrandID = '$ii_brandiddelete'");
    $product->execute();
    ...
    if ($cproduct > 0) {
        ...
        $delete = $conn->prepare("DELETE FROM `msc_brands` WHERE PK_mscBrands = '$ii_brandiddelete'");
        $delete->execute();
        $cdelete = $delete->rowCount();
    
        if ($cdelete > 0) {
            $response = array('status' => 200, 'message' => "Brand Deleted!");
            echo json_encode($response);
        } else {
