...
try {
    ...
    $verify = $conn->prepare("SELECT SUM(CASE WHEN installmentStatus = 'PENDING' THEN 1 ELSE 0 END) AS pendingTotal, SUM(CASE WHEN installmentStatus = 'APPROVED' THEN 1 ELSE 0 END) approvedTotal FROM  mn_installments WHERE FK_appsysUsers = '$usercode'");
    $verify->execute();
    $cverify = $verify->rowCount();
    $rverify = $verify->fetch(PDO::FETCH_ASSOC);

    if ($rverify["pendingTotal"] == 0 && $rverify["approvedTotal"] == 0) {
        ...
        $response = array('status' => 409, 'message' => "You already have a pending installment!");
        echo json_encode($response);
    }
