<?php
    ...
    try {
        ...
        $user = $conn->prepare("SELECT * FROM appsysusers WHERE isCustomer = 1 AND PK_appsysUsers = '$usercode'");
        $user->execute();
        $cuser = $user->rowCount();
        $ruser = $user->fetch(PDO::FETCH_ASSOC);

        if ($ruser["isCoordinatorApproved"] != "") {
            ...
            $response = array('status' => 500, 'message' => "Approval of the Coordinator is needed to add installment!");
            echo json_encode($response);
        }
